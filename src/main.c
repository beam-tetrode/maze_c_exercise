...
int maze_sanity_check(const matrix *const data_in)
{
    ...
    const size_t column_len = data_in->line_count;
    int start = 0;
    int exit = 0;

    for (size_t i = 1; i < column_len; i++) {
    ...
    for (size_t i = 0; i < row_len; i++) {
        ...
        char last = data_in->matrix[column_len-1][i];
        if (last != '#' && last != '^' && last != 'E') {
...
int where_to_go(const matrix *const maze, const x_y *const pos)
{
    ...
    // 1. priority: check for Exit
    // 2. priority: check for free position
    if (pos->x - 1 >= 0 && maze->matrix[pos->y][pos->x - 1] == 'E') {
        direction = LEFT;
    } else if (pos->y - 1 >= 0 && maze->matrix[pos->y - 1][pos->x] == 'E') {
        direction = UP;
    } else if (pos->x + 1 < maze->line_length && maze->matrix[pos->y][pos->x + 1] == 'E') {
        direction = RIGHT;
    } else if (pos->y + 1 < maze->line_count && maze->matrix[pos->y + 1][pos->x] == 'E') {
        direction = DOWN;
...
int move_in_maze(int moves, const matrix *const data_in, x_y *const coordinates)
{
    ...
    matrix data = {.matrix = NULL, .line_count = data_in->line_count,
                   .line_length = data_in->line_length};
